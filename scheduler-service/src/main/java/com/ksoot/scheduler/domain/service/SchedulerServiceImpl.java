...
import com.ksoot.scheduler.common.DateTimeHelper;
...
public class SchedulerServiceImpl implements SchedulerService {
    ...
    public Boolean deleteJob(final JobKey jobKey) {
        try {
            ...
            log.info("Job with key: {} {}", jobKey, isDeleted ? "deleted successfully"
    ...
    public Boolean deleteJobs(final JobKey... jobKeys) {
        try {
            ...
            log.info("Jobs with keys: {} {}", jobKeysList, isDeleted ? "deleted successfully"
                    : "could not be deleted");
            return isDeleted;
        } catch (final SchedulerException e) {
            throw new com.ksoot.scheduler.common.SchedulerException(e);
    ...
    public OffsetDateTime scheduleJob(final JobDetail jobDetail, final Trigger trigger) {
        try {
            ...
            if (nextFireTime != null) {
                log.info("Job with key: {} scheduled successfully with Trigger key: {}",
                        jobDetail.getKey(), trigger.getKey());
            } else {
                log.info("Job with key: {} could not be scheduled with Trigger key: {}",
                        jobDetail.getKey(), trigger.getKey());
            }
            return nextFireTime == null ? null : DateTimeHelper.offsetDateTime(nextFireTime);
        } catch (final SchedulerException e) {
            throw new com.ksoot.scheduler.common.SchedulerException(e);
        }
    }
     ...
     */
    @Override
    public Boolean scheduleJob(final JobDetail jobDetail, boolean replace, final Trigger... triggers) {
        try {
            this.scheduler.scheduleJob(jobDetail, Sets.newHashSet(triggers), replace);
            log.info("Job with key: {} scheduled successfully with given triggers",
                    jobDetail.getKey());
            return true;
        } catch (final SchedulerException e) {
            throw new com.ksoot.scheduler.common.SchedulerException(e);
    ...
    public OffsetDateTime reScheduleJob(final Trigger newTrigger) {
        try {
            ...
            if (nextFireTime != null) {
                log.info("Job with key: {} scheduled successfully with Trigger key: {}",
                        newTrigger.getJobKey(), newTrigger.getKey());
            } else {
                log.info("Job with key: {} could not be scheduled with Trigger key: {}",
                        newTrigger.getJobKey(), newTrigger.getKey());
            }
    ...
    public OffsetDateTime reScheduleJob(final TriggerKey oldTriggerKey, final Trigger newTrigger) {
        try {
            ...
            if (nextFireTime != null) {
                log.info("Job with key: {} rescheduled successfully with Trigger key: {}",
                        oldTriggerKey, newTrigger.getKey());
            } else {
                log.info("Job with key: {} could not be rescheduled with Trigger key: {}",
                        oldTriggerKey, newTrigger.getKey());
            }
            return nextFireTime == null ? null : DateTimeHelper.offsetDateTime(nextFireTime);
        } catch (final SchedulerException e) {
    ...
    public Boolean unScheduleJob(final TriggerKey triggerKey) {
        try {
            ...
            log.info("Job {} for trigger key: {}", isUnScheduled ? "unscheduled successfully"
                    : "could not be unscheduled", triggerKey);
    ...
    public Boolean unScheduleJobs(final TriggerKey... triggerKeys) {
        ...
        try {
            ...
            log.info("Job {} for trigger keys: {}", isUnScheduled ? "unscheduled successfully"
                    : "could not be unscheduled", triggerKeysList);
            return isUnScheduled;
        } catch (final SchedulerException e) {
            throw new com.ksoot.scheduler.common.SchedulerException(e);
        }
    ...
    public List<Trigger> getAllTriggers(GroupMatcher<TriggerKey> triggerKeyMatcher) {
        try {
            return this.scheduler.getTriggerKeys(triggerKeyMatcher).stream().map(this::getTrigger).toList();
        } catch (final SchedulerException e) {
            throw new com.ksoot.scheduler.common.SchedulerException(e);
        }
    ...
    public List<TriggerKey> recoverErrorTriggers() {
        try {
            return this.scheduler.getTriggerKeys(GroupMatcher.anyTriggerGroup()).stream().filter(triggerKey -> {
                try {
                    if (this.scheduler.getTriggerState(triggerKey) == TriggerState.ERROR) {
                        this.scheduler.resetTriggerFromErrorState(triggerKey);
                        log.info("Tigger with key: {} recovered from Error state successfully",
                                triggerKey);
                        return true;
                    } else {
                        return false;
                    ...
                    log.error("Tigger with key: " + triggerKey
                            + " could not be recovered from Error state", e);
                    return false;
                }
            }).toList();
    ...
    @Override
    public Boolean addCalendar(final String name, final Calendar calendar,
